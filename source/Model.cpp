...
Model::Model(const wchar_t *full_path) : Resource(full_path)
{
	...
	this->m_meshes.reserve(shapes.size());
	...
	for(size_t m = 0; m < materials.size(); m++)
	{
		for(size_t s = 0; s < shapes.size(); s++)
		{
			...
			auto vertex_buffer = GraphicsEngine::get()->getRenderSystem()->createVertexBuffer(&list_vertices[0], sizeof(VertexMesh), (UINT)list_vertices.size(), shader_byte_code, size_shader);
			auto index_buffer = GraphicsEngine::get()->getRenderSystem()->createIndexBuffer(&list_indices[0], (UINT)list_indices.size());

			this->m_meshes.push_back(std::make_shared<Mesh>(vertex_buffer, index_buffer, m));
		}
	}
}
...
const std::vector<MeshPtr> &Model::getMeshes()
{
